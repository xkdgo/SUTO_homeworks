...
class LogisticRegression:
    ...
    def train(self, X, y, learning_rate=1e-3, reg=1e-5, num_iters=100,
        ...
        for it in range(num_iters):
            ...
            #########################################################################
            iter_idx = np.random.choice(num_train, size=batch_size, replace=True)
            X_batch = X[iter_idx]
            y_batch = y[iter_idx]
            #########################################################################
            #                       END OF YOUR CODE                                #
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        hX = self.sigmoid(X * self.w)
        y_proba = np.vstack((1 - hX, hX)).T
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        hX = self.sigmoid(X_batch * self.w)
        costX = (-y_batch) * np.log(hX) -  (1.0 - y_batch) * np.log(1.0 - hX)
        theta_j = self.w[:-1]
        m = X_batch.shape[0]
        ...
        loss = (1 / m) * np.sum(costX) + (reg / (2 * m)) * np.sum(theta_j ** 2)
        dw[:-1] = ((1 / m) * (((hX - y_batch) * X_batch)))[:-1] +\
            (reg / m) * theta_j
